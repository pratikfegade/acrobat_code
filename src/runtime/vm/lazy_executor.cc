...
NDArray CreatePointerNDArray2(const std::vector<OpNode*>& nodes, int arg_num) {
  ...
  auto data_size = GetDataSize(*(nodes[0]->args_[arg_num]));
  if (nodes[0]->args_[arg_num]->data != nullptr) {
    for (size_t j = 0; j < size; ++j) {
      ICHECK(nodes[j]->args_[arg_num]->data != nullptr) << arg_num << " " << j;
      raw_data[j] = nodes[j]->args_[arg_num]->data;
    }
  } else {
    void* start = malloc(size * data_size);
    for (size_t j = 0; j < size; ++j) {
      raw_data[j] = start;
      nodes[j]->args_[arg_num]->data = start;
      start += data_size;
    }
  }
...
void LazyExecutor::BatchedExecute(bool coarsened_execution, bool all_nodes_same_depth) {
  ...
  if (all_nodes_same_depth) {
    ...
    for (int j = 0; j <= graph_depth; ++j) {
      std::unordered_map<int, std::vector<int>> func_to_node;
      for (auto node : depth_to_node[j]) {
        func_to_node[node->func_idx_].push_back(node->id_);
      }
    }

    std::vector<std::unordered_map<int, std::vector<OpNode*>>> func_to_node_vecs;
    for (int j = 0; j <= graph_depth; ++j) {
      ...
      for (auto& kv : func_to_node) {
        ...
        for (Index i = 0; i < arity; ++i) {
          switch (arg_modes[i]) {
            ...
            case kReuse: {
              if (func_nodes[0]->args_[i]->data == nullptr) {
                auto ptr = malloc(GetDataSize((*func_nodes[0]->args_[i])));
            ...
            case kScatter: {
              arg_holder[i] = CreatePointerNDArray2(func_nodes, i);
