...
class VMAOTFunctionCompiler : SourcePrinter {
 ...
 private:
        ...
        switch (instr.op) {
          ...
          case Opcode::GetField: {
            ...
            if (register_types_.at(instr.object).as<TupleTypeNode>()) {
              ...
              // << ";\n";
              stream_ << "*>(" << object_var << ")->" << GetFieldName(instr.field_index) << ";\n";
            }
            break;
          }
          case Opcode::GetTag: {
          ...
          case Opcode::AllocADT: {
            ...
            if (instr.constructor_tag == 0) {
              ...
              stream_ << dst_var << " = tvm::runtime::vm::Arena::Current()->make<std::tuple<"
              ...
              stream_ << dst_var << " = static_cast<" << concrete_type_without_shptr
                      << "*>(tvm::runtime::vm::Arena::Current()->make<"
                      << concrete_constructor_without_shptr << ">());\n";
  ...
  void EmitPMapCPP(bool definition) {
    ...
    if (ConcurrentExecution()) {
      stream_ << "List<B>* pmap(std::function<B(A, int, int&)> local_0, ";
    } else {
      stream_ << "List<B>* pmap(std::function<B(A, int&)> local_0, ";
    }
    stream_ << "List<A>* ";
    if (ConcurrentExecution()) {
    ...
    if (definition) {
      auto initializations =
          ...
          "  auto nil_node = "
          "static_cast<List<B>*>(tvm::runtime::vm::Arena::Current()->make<Nil<B>>());\n"
      ...
      auto non_concurrent_body =
          ...
          "  auto new_node = "
          "static_cast<List<B>*>(tvm::runtime::vm::Arena::Current()->make<Cons<B>>());\n"
          "  new_node->tag = LIST_CONS_TAG;\n"
          "  static_cast<Cons<B>*>(new_node)->field_0 =\n" +
          std::string(
              (ConcurrentExecution()
                   ? "      local_0(static_cast<Cons<A>*>(current)->field_0, fiber_id, "
                     "tmp_depth);\n"
                   : "      local_0(static_cast<Cons<A>*>(current)->field_0, tmp_depth);\n")) +
          "  depth = std::max(depth, tmp_depth);\n"
          "  if (new_list_tail->tag != LIST_NIL_TAG) {\n"
          "    static_cast<Cons<B>*>(new_list_tail)->field_1 = new_node;\n"
          "  } else {\n"
          "    new_list_head = new_node;\n"
          "  }\n"
          "  static_cast<Cons<B>*>(new_node)->field_1 = nil_node;\n"
          "  new_list_tail = new_node;\n"
          "  current = static_cast<Cons<A>*>(current)->field_1;\n"
          "}\n";
      ...
      auto concurrent_body =
          ...
          "    static_cast<Cons<B>*>(new_node)->field_0 =\n"
          "        local_0(static_cast<Cons<A>*>(current)->field_0, new_id, tmp_depth);\n"
