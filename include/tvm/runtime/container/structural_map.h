 ...
 * \brief Runtime StructuralMap container types.
 */
#ifndef TVM_RUNTIME_CONTAINER_STRUCTURAL_MAP_H_
#define TVM_RUNTIME_CONTAINER_STRUCTURAL_MAP_H_
...
class StructuralMapNode : public Object {
 public:
  ...
  using ContainerType = std::unordered_map<ObjectRef, ObjectRef, StructuralHash, StructuralEqual>;
  ...
  static constexpr const uint32_t _type_index = runtime::TypeIndex::kRuntimeStructuralMap;
  static constexpr const char* _type_key = "StructuralMap";
  TVM_DECLARE_FINAL_OBJECT_INFO(StructuralMapNode, Object);

  /*!
   * \brief Number of elements in the SmallStructuralMapNode
   * \return The result
   */
   ...
   */
  static ObjectPtr<StructuralMapNode> Empty() { return make_object<StructuralMapNode>(); }
 ...
 protected:
  ...
  static ObjectPtr<Object> CreateFromRange(IterType first, IterType last) {
    ObjectPtr<StructuralMapNode> p = make_object<StructuralMapNode>();
  ...
  static void InsertMaybeReHash(const KVType& kv, ObjectPtr<Object>* map) {
    StructuralMapNode* map_node = static_cast<StructuralMapNode*>(map->get());
    map_node->data_[kv.first] = kv.second;
  }
  /*!
   * \brief Create an empty container with elements copying from another StructuralMapNode
   * \param from The source container
   * \return The object created
   */
  static ObjectPtr<StructuralMapNode> CopyFrom(StructuralMapNode* from) {
    ObjectPtr<StructuralMapNode> p = make_object<StructuralMapNode>();
...
class StructuralMapNode : public Object {
 public:
  ...
  class iterator {
   ...
   protected:
    ...
    iterator(uint64_t index, const StructuralMapNode* self) : index(index), self(self) {}
    /*! \brief The position on the array */
    uint64_t index;
    /*! \brief The container it points to */
    const StructuralMapNode* self;

    friend class DenseStructuralMapNode;
    friend class SmallStructuralMapNode;
  };
  /*!
   * \brief Create an empty container
   * \return The object created
   */
  static inline ObjectPtr<StructuralMapNode> Empty();
 ...
 protected:
  ...
  /*!
   * \brief Create an empty container with elements copying from another SmallStructuralMapNode
   * \param from The source container
   * \return The object created
   */
  static inline ObjectPtr<StructuralMapNode> CopyFrom(StructuralMapNode* from);
...
class SmallStructuralMapNode
    : public StructuralMapNode,
      public runtime::InplaceArrayBase<SmallStructuralMapNode, StructuralMapNode::KVType> {
 private:
  static constexpr uint64_t kInitSize = 2;
  static constexpr uint64_t kMaxSize = 4;

 public:
  using StructuralMapNode::iterator;
  using StructuralMapNode::KVType;

  /*! \brief Defaults to the destructor of InplaceArrayBase */
  ~SmallStructuralMapNode() = default;
  /*!
   * \brief Count the number of times a key exists in the SmallStructuralMapNode
   * \param key The indexing key
   * \return The result, 0 or 1
   */
  ...
  const mapped_type& at(const key_type& key) const {
    ...
    ICHECK(itr.index < size_) << "IndexError: key is not in StructuralMap";
    return itr->second;
  }
  /*!
   * \brief Index value associated with a key, throw exception if the key does not exist
 ...
 private:
  /*!
   * \brief Remove a position in SmallStructuralMapNode
   ...
   * \return The object created
   */
  static ObjectPtr<SmallStructuralMapNode> Empty(uint64_t n = kInitSize) {
    using ::tvm::runtime::make_inplace_array_object;
    ObjectPtr<SmallStructuralMapNode> p =
        make_inplace_array_object<SmallStructuralMapNode, KVType>(n);
    p->size_ = 0;
    p->slots_ = n;
    return p;
  }
   ...
   */
  template <typename IterType>
  static ObjectPtr<SmallStructuralMapNode> CreateFromRange(uint64_t n, IterType first,
                                                           IterType last) {
    ObjectPtr<SmallStructuralMapNode> p = Empty(n);
   ...
   * \return The object created
   */
  static ObjectPtr<SmallStructuralMapNode> CopyFrom(SmallStructuralMapNode* from) {
   ...
   */
  static void InsertMaybeReHash(const KVType& kv, ObjectPtr<Object>* map) {
    SmallStructuralMapNode* map_node = static_cast<SmallStructuralMapNode*>(map->get());
 ...
 protected:
  friend class StructuralMapNode;
  friend class DenseStructuralMapNode;
  friend class runtime::InplaceArrayBase<SmallStructuralMapNode, StructuralMapNode::KVType>;
};
 ...
 * A. Overview
 *
 * DenseStructuralMapNode did several improvements over traditional separate chaining hash,
 ...
 * relatively short (length <= 3) in hash maps, we follow [1]'s idea that only allows the pointer
 * to be one of the 126 possible values, i.e. if the next element of i-th slot is (i + x)-th
 * element, then x must be one of the 126 pre-defined values.
 *
 ...
 *
 * B3. InsertMaybeReHash an element. Following B2, we first traverse the linked list to see if
 * this element is in the linked list, and if not, we put it at the end by probing the next empty
 ...
 * follow the suggestion in [1] that also use triangle numbers for "next pointer" as well as
 * sparing for list head.
 *
 ...
 */
class DenseStructuralMapNode : public StructuralMapNode {
 private:
 ...
 public:
  ...
  /*!
   * \brief Destroy the DenseStructuralMapNode
   */
  ~DenseStructuralMapNode() { this->Reset(); }
 ...
 private:
  ...
  mapped_type& At(const key_type& key) const {
    ...
    ICHECK(!iter.IsNone()) << "IndexError: key is not in StructuralMap";
    return iter.Val();
  }
  /*!
   ...
   * exist, but the slot of its head has been occupied by other linked lists. In this case, we
   * need to spare the slot by moving away the elements to another valid empty one to make
   * insertion possible. \param target The given entry to be spared \param key The indexing key
  ...
  /*! \brief Release the memory acquired by the container without deleting its entries stored
   * inside
   */
  void ReleaseMemory() {
    delete[] data_;
   ...
   * \return The object created
   */
  static ObjectPtr<DenseStructuralMapNode> Empty(uint32_t fib_shift, uint64_t n_slots) {
    ICHECK_GT(n_slots, uint64_t(SmallStructuralMapNode::kMaxSize));
    ObjectPtr<DenseStructuralMapNode> p = make_object<DenseStructuralMapNode>();
  ...
  /*!
   * \brief Create an empty container with elements copying from another DenseStructuralMapNode
   * \param from The source container
   * \return The object created
   */
  static ObjectPtr<DenseStructuralMapNode> CopyFrom(DenseStructuralMapNode* from) {
    ObjectPtr<DenseStructuralMapNode> p = make_object<DenseStructuralMapNode>();
   ...
   */
  static void InsertMaybeReHash(const KVType& kv, ObjectPtr<Object>* map) {
    DenseStructuralMapNode* map_node = static_cast<DenseStructuralMapNode*>(map->get());
    ListNode iter;
    ...
    ICHECK_GT(map_node->slots_, uint64_t(SmallStructuralMapNode::kMaxSize));
  ...
  struct ListNode {
    ...
    ListNode(uint64_t index, const DenseStructuralMapNode* self)
    ...
    bool MoveToNext(const DenseStructuralMapNode* self, uint8_t meta) {
    ...
    /*! \brief Move the entry to the next entry on the linked list */
    bool MoveToNext(const DenseStructuralMapNode* self) { return MoveToNext(self, Meta()); }
    /*! \brief Get the previous entry on the linked list */
    ListNode FindPrev(const DenseStructuralMapNode* self) const {
    ...
    bool GetNextEmpty(const DenseStructuralMapNode* self, uint8_t* jump, ListNode* result) const {
...
#define TVM_DISPATCH_MAP(base, var, body)            \
  {                                                  \
    using TSmall = SmallStructuralMapNode*;          \
    using TDense = DenseStructuralMapNode*;          \
    uint64_t slots = base->slots_;                   \
    if (slots <= SmallStructuralMapNode::kMaxSize) { \
...
#define TVM_DISPATCH_MAP_CONST(base, var, body)      \
  {                                                  \
    using TSmall = const SmallStructuralMapNode*;    \
    using TDense = const DenseStructuralMapNode*;    \
    uint64_t slots = base->slots_;                   \
    if (slots <= SmallStructuralMapNode::kMaxSize) { \
      TSmall var = static_cast<TSmall>(base);        \
      body;                                          \
...
inline StructuralMapNode::iterator::pointer StructuralMapNode::iterator::operator->() const {
  TVM_DISPATCH_MAP_CONST(self, p, { return p->DeRefItr(index); });
}

inline StructuralMapNode::iterator& StructuralMapNode::iterator::operator++() {
...
inline size_t StructuralMapNode::count(const key_type& key) const {
  TVM_DISPATCH_MAP_CONST(this, p, { return p->count(key); });
}

inline const StructuralMapNode::mapped_type& StructuralMapNode::at(
    const StructuralMapNode::key_type& key) const {
  TVM_DISPATCH_MAP_CONST(this, p, { return p->at(key); });
}

inline StructuralMapNode::mapped_type& StructuralMapNode::at(
    const StructuralMapNode::key_type& key) {
  TVM_DISPATCH_MAP(this, p, { return p->at(key); });
}

inline StructuralMapNode::iterator StructuralMapNode::begin() const {
  TVM_DISPATCH_MAP_CONST(this, p, { return p->begin(); });
}

inline StructuralMapNode::iterator StructuralMapNode::end() const {
  TVM_DISPATCH_MAP_CONST(this, p, { return p->end(); });
}

inline StructuralMapNode::iterator StructuralMapNode::find(
    const StructuralMapNode::key_type& key) const {
  TVM_DISPATCH_MAP_CONST(this, p, { return p->find(key); });
}

inline void StructuralMapNode::erase(const StructuralMapNode::iterator& position) {
...
inline ObjectPtr<StructuralMapNode> StructuralMapNode::Empty() {
  return SmallStructuralMapNode::Empty();
}

inline ObjectPtr<StructuralMapNode> StructuralMapNode::CopyFrom(StructuralMapNode* from) {
  if (from->slots_ <= SmallStructuralMapNode::kMaxSize) {
    return SmallStructuralMapNode::CopyFrom(static_cast<SmallStructuralMapNode*>(from));
  } else {
    return DenseStructuralMapNode::CopyFrom(static_cast<DenseStructuralMapNode*>(from));
  }
}

template <typename IterType>
inline ObjectPtr<Object> StructuralMapNode::CreateFromRange(IterType first, IterType last) {
  int64_t _cap = std::distance(first, last);
  if (_cap < 0) {
    return SmallStructuralMapNode::Empty();
  }
  uint64_t cap = static_cast<uint64_t>(_cap);
  if (cap < SmallStructuralMapNode::kMaxSize) {
    return SmallStructuralMapNode::CreateFromRange(cap, first, last);
  }
  uint32_t fib_shift;
  uint64_t n_slots;
  DenseStructuralMapNode::CalcTableSize(cap, &fib_shift, &n_slots);
  ObjectPtr<Object> obj = DenseStructuralMapNode::Empty(fib_shift, n_slots);
  for (; first != last; ++first) {
    KVType kv(*first);
    DenseStructuralMapNode::InsertMaybeReHash(kv, &obj);
  }
  return obj;
}

inline void StructuralMapNode::InsertMaybeReHash(const KVType& kv, ObjectPtr<Object>* map) {
  constexpr uint64_t kSmallStructuralMapMaxSize = SmallStructuralMapNode::kMaxSize;
  StructuralMapNode* base = static_cast<StructuralMapNode*>(map->get());
  if (base->slots_ < kSmallStructuralMapMaxSize) {
    SmallStructuralMapNode::InsertMaybeReHash(kv, map);
  } else if (base->slots_ == kSmallStructuralMapMaxSize) {
    if (base->size_ < base->slots_) {
      SmallStructuralMapNode::InsertMaybeReHash(kv, map);
    } else {
      ObjectPtr<Object> new_map = StructuralMapNode::CreateFromRange(base->begin(), base->end());
      DenseStructuralMapNode::InsertMaybeReHash(kv, &new_map);
      *map = std::move(new_map);
    }
  } else {
    DenseStructuralMapNode::InsertMaybeReHash(kv, map);
  }
}

template <>
inline ObjectPtr<StructuralMapNode> make_object<>() = delete;

#endif

/*!
 * \brief StructuralMap container of NodeRef->NodeRef in DSL graph.
 *  StructuralMap implements copy on write semantics, which means map is mutable
...
class StructuralMap : public ObjectRef {
 public:
  using key_type = K;
  using mapped_type = V;
  class iterator;
   ...
   * \brief default constructor
   */
  StructuralMap() { data_ = StructuralMapNode::Empty(); }
  /*!
   * \brief move constructor
   * \param other source
   */
  StructuralMap(StructuralMap<K, V>&& other) { data_ = std::move(other.data_); }
  /*!
   * \brief copy constructor
   * \param other source
   */
  StructuralMap(const StructuralMap<K, V>& other) : ObjectRef(other.data_) {}
  /*!
   * \brief copy assign operator
   * \param other The source of assignment
   * \return reference to self.
   */
  StructuralMap<K, V>& operator=(StructuralMap<K, V>&& other) {
   ...
   * \return reference to self.
   */
  StructuralMap<K, V>& operator=(const StructuralMap<K, V>& other) {
    data_ = other.data_;
    return *this;
  }
  /*!
   ...
   */
  explicit StructuralMap(ObjectPtr<Object> n) : ObjectRef(n) {}
  /*!
   ...
   */
  template <typename IterType>
  StructuralMap(IterType begin, IterType end) {
    data_ = StructuralMapNode::CreateFromRange(begin, end);
  }
  /*!
   * \brief constructor from initializer list
   * \param init The initalizer list
   */
  StructuralMap(std::initializer_list<std::pair<K, V>> init) {
    data_ = StructuralMapNode::CreateFromRange(init.begin(), init.end());
  }
  /*!
  ...
  StructuralMap(const std::unordered_map<K, V, Hash, Equal>& init) {  // NOLINT(*)
    data_ = StructuralMapNode::CreateFromRange(init.begin(), init.end());
  }
  /*!
   ...
   */
  const V at(const K& key) const { return DowncastNoCheck<V>(GetStructuralMapNode()->at(key)); }
  /*!
   * \brief Read element from map.
   * \param key The key
   * \return the corresonding element.
  ...
  size_t size() const {
    StructuralMapNode* n = GetStructuralMapNode();
    return n == nullptr ? 0 : n->size();
  }
  /*! \return The number of elements of the key */
  size_t count(const K& key) const {
    StructuralMapNode* n = GetStructuralMapNode();
    return n == nullptr ? 0 : GetStructuralMapNode()->count(key);
  }
  ...
  void clear() {
    StructuralMapNode* n = GetStructuralMapNode();
    if (n != nullptr) {
      data_ = StructuralMapNode::Empty();
    }
  }
  /*!
   * \brief set the StructuralMap.
   * \param key The index key.
   * \param value The value to be setted.
   */
  void Set(const K& key, const V& value) {
    CopyOnWrite();
    StructuralMapNode::InsertMaybeReHash(StructuralMapNode::KVType(key, value), &data_);
  }
  /*! \return begin iterator */
  iterator begin() const { return iterator(GetStructuralMapNode()->begin()); }
  /*! \return end iterator */
  iterator end() const { return iterator(GetStructuralMapNode()->end()); }
  /*! \return find the key and returns the associated iterator */
  iterator find(const K& key) const { return iterator(GetStructuralMapNode()->find(key)); }
  /*! \return The value associated with the key, NullOpt if not found */
  Optional<V> Get(const K& key) const {
    StructuralMapNode::iterator iter = GetStructuralMapNode()->find(key);
    if (iter == GetStructuralMapNode()->end()) {
      return NullOptType{};
    }
   ...
   */
  StructuralMapNode* CopyOnWrite() {
    if (data_.get() == nullptr) {
      data_ = StructuralMapNode::Empty();
    } else if (!data_.unique()) {
      data_ = StructuralMapNode::CopyFrom(GetStructuralMapNode());
    }
    return GetStructuralMapNode();
  }
  /*! \brief specify container node */
  using ContainerType = StructuralMapNode;
  ...
  class iterator {
   ...
   private:
    iterator(const StructuralMapNode::iterator& itr)  // NOLINT(*)
        : itr(itr) {}

    template <typename, typename, typename, typename>
    friend class StructuralMap;

    StructuralMapNode::iterator itr;
  };

 private:
  /*! \brief Return data_ as type of pointer of StructuralMapNode */
  StructuralMapNode* GetStructuralMapNode() const {
    return static_cast<StructuralMapNode*>(data_.get());
  }
};

/*!
 * \brief Merge two StructuralMaps.
 * \param lhs the first StructuralMap to merge.
 * \param rhs the second StructuralMap to merge.
 * @return The merged Array. Original StructuralMaps are kept unchanged.
 */
template <typename K, typename V,
          typename = typename std::enable_if<std::is_base_of<ObjectRef, K>::value>::type,
          typename = typename std::enable_if<std::is_base_of<ObjectRef, V>::value>::type>
inline StructuralMap<K, V> Merge(StructuralMap<K, V> lhs, const StructuralMap<K, V>& rhs) {
...
using runtime::StructuralMap;
using runtime::StructuralMapNode;
}  // namespace tvm
