...
class Z3Converter : public tir::ExprFunctor<z3expr(const PrimExpr&)> {
 public:
  ...
  z3fun GetOrCreateZ3Fun(const RelayExpr& f, const std::string& name, int arity);
  ...
  z3expr VisitExpr_(const ScatterLoadNode* op) override;
  z3expr VisitRightShift(const CallNode* op);
  z3expr VisitLeftShift(const CallNode* op);
...
#undef BINOP_DECLARE_CONVERTER_FUN
 ...
 private:
  z3::context& ctx;
  std::unordered_map<PrimExpr, z3expr, ObjectPtrHash, ObjectPtrEqual> z3_exprs;
  std::unordered_map<ObjectRef, z3fun, ObjectPtrHash, ObjectPtrEqual> z3_funs;
  int index = 0;
};
